...
def build_unsupervised_graph() -> StateGraph:
    """Clustering pipeline. Shares cleaning/EDA/visualization/feature
    engineering/refinement/plan-review with the supervised graph via
    _add_shared_nodes_and_entry_edges, but model_selection,
    hyperparameter_tuning, and evaluation route to clustering-specific
    nodes instead — chosen via route_task_unsupervised rather than
    route_task, even though the underlying task-name STRINGS
    ("model_selection", "hyperparameter_tuning", "evaluation") are
    identical across both graphs, by design (keeps STAGE_LABELS,
    completed_tasks display, and STAGE_FIELDS clearing logic uniform
    regardless of which family actually ran).
    """
    builder = StateGraph(GraphState)
    _add_shared_nodes_and_entry_edges(builder)
    ...
    builder.add_node("feature_engineering_planner",
                     feature_engineering_planner_node)
    builder.add_node("feature_engineering", feature_engineering_node)
    builder.add_node("clustering_model_selection_planner",
                     clustering_model_selection_planner_node)
    builder.add_node("enqueue_clustering", enqueue_clustering_node)
    builder.add_node("poll_clustering", poll_clustering_node)
    builder.add_node("enqueue_clustering_tuning",
                     enqueue_clustering_tuning_node)
    builder.add_node("poll_clustering_tuning", poll_clustering_tuning_node)
    builder.add_node("clustering_evaluation", clustering_evaluation_node)
    builder.add_node("reporting", reporting_node)
    ...
    builder.add_conditional_edges(
        ...
        route_task_unsupervised,
        {
            ...
            "visualization": "visualization_planner",
            "feature_engineering": "feature_engineering_planner",
            "model_selection": "clustering_model_selection_planner",
            "hyperparameter_tuning": "enqueue_clustering_tuning",
    ...
    builder.add_edge("feature_engineering_planner", "feature_engineering")
    builder.add_edge("feature_engineering", "router")
    builder.add_edge("clustering_model_selection_planner",
                     "enqueue_clustering")
    builder.add_edge("enqueue_clustering", "poll_clustering")
    builder.add_edge("poll_clustering", "router")
    builder.add_edge("enqueue_clustering_tuning", "poll_clustering_tuning")
    builder.add_edge("poll_clustering_tuning", "router")
    builder.add_edge("clustering_evaluation", "router")
    builder.add_edge("reporting", "router")
