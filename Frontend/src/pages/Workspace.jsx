...
import { formatAnalysisAsMarkdown } from "../utils/FormatReport";
...
function Composer({ onStart }) {
  ...
  <div className="bg-white border border-muted/20 rounded-md p-4 max-h-64 overflow-auto">
    <article className="prose prose-sm max-w-none prose-headings:font-serif prose-headings:text-ink prose-p:text-ink prose-li:text-ink prose-strong:text-ink">
      <ReactMarkdown>{formatAnalysisAsMarkdown(quickResult)}</ReactMarkdown>
    </article>
  </div>
)}
...
function InterruptCard({ interrupt }) {
  ...
  // Fallback for any future/unknown interrupt type — still readable,
  // not a raw JSON dump, using summary if present.
  return (
    <div className="bg-white border border-muted/20 rounded-lg px-4 py-3 text-sm max-w-[85%]">
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div className={`max-w-[80%] rounded-lg px-4 py-2 text-sm ${
        ...
        {msg.type === "done" ? <p>Pipeline step finished.</p> : <p className="whitespace-pre-wrap">{msg.text}</p>}
      </div>
    </div>
  );
}
...
function Workspace() {
  ...
  async function startRun(file, query) {
    ...
    setMessages([{ role: "user", type: "text", text: query }, ...resultToMessages(runRes.data)]);
    setAwaitingDecision(Boolean(runRes.data.interrupted));
    setReportReady(!runRes.data.interrupted);
  }
  ...
  if (!threadId) return <Composer onStart={startRun} />;
